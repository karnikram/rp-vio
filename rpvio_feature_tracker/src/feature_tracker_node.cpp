...
void callback(const sensor_msgs::ImageConstPtr &img_msg, const sensor_msgs::ImageConstPtr &mask_msg)
{
   ...
   if (PUB_THIS_FRAME)
   {
        ...
        if (SHOW_TRACK)
        {
            ...
            cv_bridge::CvImageConstPtr orig_mask_ptr = cv_bridge::toCvCopy(mask_msg, sensor_msgs::image_encodings::BGR8);
            cv::Mat mask_img = orig_mask_ptr->image;

            for (int i = 0; i < NUM_OF_CAM; i++)
            {
                cv::Mat tmp_img = stereo_img.rowRange(i * ROW, (i + 1) * ROW);
                cv::cvtColor(show_img, tmp_img, CV_GRAY2RGB);
                cv::addWeighted(tmp_img, 1.0, mask_img, 0.3, 0.0, tmp_img);
