...
void FeatureTracker::convertSegToPlane(cv::Mat &_mask)
{
    for(size_t i = 0; i < _mask.rows; i++)
        for(size_t j = 0; j < _mask.cols; j++)
        {
            cv::Vec3b intensity = _mask.at<cv::Vec3b>(i,j);
            if(std::count(PLANES_BGR.begin(), PLANES_BGR.end(), intensity) == 0)
                _mask.at<cv::Vec3b>(i,j) = cv::Vec3b(0,0,0);
        }

    cv::cvtColor(_mask, _mask, cv::COLOR_BGR2GRAY);
}
...
void FeatureTracker::addPoints(const cv::Mat &_mask)
{
    ...
    convertSegToPlane(mask_copy);
    cv::Mat gray_mask_copy = mask_copy.clone();
    ...
    for(auto &label : unique_labels)
        unique_masks.emplace(label, cv::Mat::zeros(gray_mask_copy.size(), CV_8UC1));

    for(int i = 0; i < gray_mask_copy.rows; i++)
        for(int j = 0; j < gray_mask_copy.cols; j++)
            unique_masks[gray_mask_copy.at<uchar>(i,j)].at<uchar>(i,j) = 255;
